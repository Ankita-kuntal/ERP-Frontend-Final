...
const FacultyProfile: React.FC = () => {
  ...
  const isFaculty = (user: any): user is Faculty => {
    console.log("Checking faculty type with user:", user);
    ...
    console.log("Is faculty type:", isFacultyType);
  ...
  const handleUserInfoSubmit = async (data: FormData) => {
    try {
      ...
      for (let pair of new_data.entries()) {
  ...
  if (!isFaculty(user)) {
    console.log("User is not a faculty type. User data:", user);
    return (
      <div className="flex items-center justify-center h-screen">
  ...
  console.log("Rendering faculty profile with data:", user);
