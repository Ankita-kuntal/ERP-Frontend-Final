...
// Add this type assertion for roles
const roleNames: Record<string, string> = conf.college.roles as Record<string, string>;
// Extend PendingScholarship to include role
...
export const ApproveScholarship = () => {
  ...
  const [pendingScholarships, setPendingScholarships] = useState<
    Record<string, ExtendedPendingScholarship[]>
  >({});
  const [selectedScholarship, setSelectedScholarship] =
    useState<ExtendedPendingScholarship | null>(null);
  const [baseScholarship, setBaseScholarship] =
    useState<ExtendedPendingScholarship | null>(null);
      ...
      try {
        ...
        const transformedData = Object.entries(response).reduce(
          (acc, [role, scholarships]) => {
            // Assert scholarships as PendingScholarship[]
            const typedScholarships = scholarships as PendingScholarship[];
            acc[role] = typedScholarships.map((scholarship: PendingScholarship) => ({
              ...scholarship,
              role,
              selected: false,
              days: 0,
  ...
  const handleSelectAll = (role: string) => {
    setPendingScholarships((prev) => {
      const newSelectedState = !selectAll[role];
      return {
        ...prev,
  ...
  const handleScholarshipClick = (scholarship: ExtendedPendingScholarship) => {
    setSelectedScholarship(scholarship);
    setBaseScholarship(scholarship);
  ...
  const handleBulkApprove = async (role: string) => {
    ...
    try {
      ...
      if (allSuccessful) {
        setPendingScholarships((prev) => ({
          ...prev,
          [role]: prev[role].filter((sch) => !sch.selected),
        }));
        setSelectedScholarship(null);
        setBaseScholarship(null);
        setComment("");
        setSelectAll((prev) => ({ ...prev, [role]: false }));

        showSuccess("All selected scholarships approved.");
      } else {
        throw new Error("Some approvals failed.");
      }
    } catch (error) {
      console.error("Bulk approval failed:", error);
      showError("Bulk approval failed. Try again.");
    }
  };
      ...
      if (response && response.success) {
        ...
        showSuccess(
          `Scholarship Successfully Accepted by ${
            selectedRole && roleNames[selectedRole]
  ...
  const handleDaysSave = async () => {
    if (!selectedScholarship || isNaN(editedDays)) return;
